...
After the report is complete, manually dispatch `Reset Talkdesk demo` with the recorded application/control PRs and merge SHAs, controls branch, completed run ID, deployment UUID, image digest, and report location. The reset reverts repository state only. The same immutable app remains running; Coolify deployment history, reports, Aiden history, and observability data remain available.
